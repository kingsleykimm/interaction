...
from habitat.config.default_structured_configs import ThirdRGBSensorConfig, HeadRGBSensorConfig, HumanoidDetectorSensorConfig, HeadDepthSensorConfig, ArmPanopticSensorConfig
import habitat
...
def make_hab_cfg(agent_dict, action_dict):
    ...
    task_config.lab_sensors = {
        'humanoid_detector' : HumanoidDetectorSensorConfig(human_id=0) # these get assigned to each agent, so remember to prepend with the agent_{id}
    }
...
def play_env(env : habitat.Env) -> None: # we are in the environment at this point
    ...
    while agent_displ > 1e-9 or agent_rot > 1e-9:
        ...
        if obs["agent_1_humanoid_detector_sensor"] == np.ones(1, dtype=np.float32):
            break
